...
PImage bg;
PImage img;
color[] orangeRainbow = {#FFFEFF, #F2BB77, #F2DD72, #F27329, #F22929};
PFont font;
...
void reset() {
  ...
  //allows plastic ball to move and bounce in different directions 
  speedX = random(5, 5);
  speedY = random(5, 5);
  ...
  //reset size of moving rectangle
  rectSize = 200;
}
...
//score
  ...
  if ( x > width-30 && x < width -20 && y > mouseY-rectSize/2 && y < mouseY+rectSize/2 ) {
    speedX = speedX * -1;
    x = x + speedX;
    fill(orangeRainbow[int(random(0, 5))]); //changes to random colour from array
  ...
  if (x < 25) {
    ...
    speedY *= 1.1;
    x += speedX;
    fill(orangeRainbow[int(random(0, 4))]); //changes to random colour from array
  }
...
//Game Over Screen
  if (x > 1500) {
    ...
    //play game over music when player loses
